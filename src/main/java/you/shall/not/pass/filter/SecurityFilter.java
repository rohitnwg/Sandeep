...
public class SecurityFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        try {
            ...
            // 
            final String cookieValue = cookieService.getCookieValue((HttpServletRequest) request, SESSION_COOKIE);
            final Optional<Session> sessionByToken = sessionService.findSessionByToken(cookieValue);
            if(sessionByToken== null) {
    	        Optional<String> optionalSession = sessionService.getDefaultSession();
    	    	
    	        optionalSession.ifPresent(session -> {
    	        	HttpServletResponse httpServletResponse = (HttpServletResponse) response;
    	        	System.out.println("newly session : " + session);
    	            cookieService.addCookie(session, httpServletResponse);
    	            httpServletResponse.addHeader("Set-Cookie", session);
    	        });
            }
            //
    ...
    private void shallNotPassLogic(HttpServletRequest request) {
        ...
        resourceValidator.ifPresent(validator -> {
            ...
            if(validator.requires() == Access.Level0) {
            	LOG.info("user grant level {}", Access.Level0+" Direct access");
            	return;
            }
            if (sessionService.isExpiredSession(sessionByToken)
            		|| validator.requires().levelIsHigher(grant)) {
            	System.out.println("3 : " + sessionService.isExpiredSession(sessionByToken));
            	System.out.println("4 : " + validator.requires().levelIsHigher(grant));
