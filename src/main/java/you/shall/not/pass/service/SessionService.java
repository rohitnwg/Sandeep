...
public class SessionService {
    ...
    public boolean isExpiredSession(Optional<Session> optionalSession) {
    	System.out.println("1 : "+!optionalSession.isPresent());
    	System.out.println("2 : "+ optionalSession.filter(session -> LocalDateTime.now()
    ...
    public Optional<String> getDefaultSession() {
    	 final String token = csrfProtectionService.generateToken();
    	 Session session = Session.builder()
                 .token(token)
                 .build();
    	 sessionRepository.save(session);
        LOG.info("returning default session :" + session);
